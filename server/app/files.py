...
async def parse_file_content_async(
    file_path: str, file_extension: str, file_md5: str, id_url_info: Dict[str, Any]
) -> None:
            ...
            try:
                ...
                try:
                    with diskcache_lock.lock():
                        cur.execute(
                            "UPDATE t_local_file_tab SET doc_status = ? WHERE id = ?",
...
async def add_files_limited_by_semaphore(
    ...
    for data in file_data:
        await parse_file_content_async(
            data["file_path"], data["file_extension"], data["file_md5"], id_url_info
        )


def add_local_file_info(
    file_data: List[Dict[str, str]],
    id_url_info: Dict[str, Any],
    max_concurrent_writes: int = 5,
):
    ...
    loop.run_until_complete(
        add_files_limited_by_semaphore(file_data, id_url_info, max_concurrent_writes)
    )
    loop.close()
...
def submit_local_file_list():
    ...
    if len(file_list) > MAX_LOCAL_FILE_BATCH_LENGTH:
        ...
        return {
            "retcode": -20001,
            "message": f"Too many files uploaded, the maximum is {MAX_LOCAL_FILE_BATCH_LENGTH}!",
        }
    ...
    try:
        for file_ in file_list:
            ...
            if file_size > MAX_FILE_SIZE:
                ...
                return {
                    "retcode": -20002,
                    "message": f"File {file_.filename} exceeds the size limit of {MAX_FILE_SIZE} bytes!",
                }
            ...
            if file_extension.lower() not in FILE_LOADER_EXTENSIONS:
                ...
                return {
                    "retcode": -20003,
                    "message": f"Unsupported file extension '{file_extension}' for {file_.filename}",
                }
            ...
            save_directory = os.path.join(
                STATIC_DIR, LOCAL_FILE_DOWNLOAD_DIR, day_folder, unique_folder
            )
        ...
        duplicate_files = [
            data["filename"] for data in file_data if data["file_md5"] in existing_md5
        ]
        if duplicate_files:
        ...
        id_url_info = {}
        for row in rows:
            id_url_info[row["content_md5"]] = {"id": row["id"], "url": row["url"]}

        Thread(
            target=add_local_file_info,
            args=(file_data, id_url_info, MAX_CONCURRENT_WRITES),
        ).start()

        return {
            "retcode": 0,
            "message": "Files uploaded and metadata saved successfully",
            "data": {"file_id_list": inserted_ids},
        }
    except Exception as e:
        logger.error(f"An error occurred: {e}")
        return {"retcode": -30000, "message": f"An error occurred: {e}", "data": {}}
...
def get_local_file_sub_content_list():
    ...
    try:
        ...
        cur.execute(
            "SELECT COUNT(*) FROM t_local_file_chunk_tab WHERE file_id = ?", (file_id,)
        )
        ...
        return {
            "retcode": 0,
            "message": "success",
            "data": {"total_count": total_count, "sub_content_list": record_list},
        }
    except Exception as e:
        logger.error(f"An error occurred: {e}")
